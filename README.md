...
```bash
composer require fleshgrinder/formatter
```
...
The internal iterator over the arguments has not been advanced by the time the
 ...
 Essentially, placeholders which explicitly name their argument do not affect
 placeholders which do not name an argument in terms of positional placeholders.

Placeholders are not limited to numbers, it is also possible to access them
 by name. Names are limited to `A-Za-z0-9_-` characters, this ensures highest
 compatibility and should match 99&thinsp;% of all associative array keys in
 the PHP world:
...
Numbers are formatted with [`number_format`](https://php.net/number-format) and
 its default values for decimal and thousands separator, this ensures best
...
Note that there are no `#E` or `#X` modifiers to force consistent output.

### Iterable Listings and Modifiers
Iterable data structures are formatted as comma-separated lists by default.
 An optional conjunction may be configured by adding the desired word after a
 colon (`:`) inside the braces after the placeholder:
...
A placeholder is marked by appending a question mark (`?`) after the closing
 ...
 be non-empty in order to be included. Note that no exception if a marked
 placeholder within an optional part is missing from the arguments, as this
 counts as being empty.
...
Any error or exception that might be thrown by an argument that is being
 ...
 but it might throw an [`InvalidArgumentException`](src/Formatter/InvalidArgumentException.php)
 as well if the type modifier is not present an argument is:
